...
for i in range(0, len(refImgs)):
    ...
    for imgName, imgLabel in zip(distImgNames, distImgLabels):
        ...
        for patch_col in range(3,imgCols-patchSize-3,int(patchSize/2.)):  # 1/3 overlap
            for patch_row in range(3,imgRows-patchSize-3,int(patchSize/2.)):  # 1/3 overlap
                refPatch = refImg[patch_row:patch_row+patchSize,patch_col:patch_col+patchSize,:]
...
if mode == "train":
    with h5py.File("./hdf5Files_train/QualityClassification_data_March31" +'.h5', 'w') as hf:
        hf.create_dataset('data', data=finalDistortPatches)
        hf.create_dataset('labels', data=finalDistortLabels)
elif mode == "val":
    with h5py.File("./hdf5Files_val/QualityClassification_data_March31" +'.h5', 'w') as hf:
        hf.create_dataset('data', data=finalDistortPatches)
        hf.create_dataset('labels', data=finalDistortLabels)
else:
    with h5py.File("./hdf5Files_test/QualityClassification_data_March31" +'.h5', 'w') as hf:
        hf.create_dataset('data', data=finalDistortPatches)
        hf.create_dataset('labels', data=finalDistortLabels)
