...
class myCallback(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        model.save_weights(weightSavePath + "bestWeights_regressMOS_lowKernels_latestModel.h5",overwrite=True)
        ...
        if mean_corr > self.best_mean_corr:
            ...
            model.save_weights(weightSavePath + "bestWeights_regressMOS_lowKernels_bestCorr.h5",overwrite=True)
...
model.load_weights(weightSavePath + "bestWeights_regressMOS_lowKernels_latestModel.h5")
printing("Latest weights loaded.")
...
checkpointer = ModelCheckpoint(filepath = weightSavePath + "bestWeights_lowKernels_bestLoss.h5", monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
cb = myCallback()
history = LossHistory()
