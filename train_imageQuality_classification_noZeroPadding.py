...
logger 			= '/media/AccessParag/Code/DNN_imageQuality_classification_Apr3.txt'
...
class myCallback(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if epoch > 0:
            # pdb.set_trace()
            past_two_acc = self.acc[-2:]
            past_two_acc_diff = np.diff(past_two_acc)
            testDecrease = np.all(past_two_acc_diff<=0)
            if testDecrease:
...
model.add(Convolution2D(64, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(MaxPooling2D(pool_size=(2,2), strides=(1,1)))  # 27 x 27
...
model.add(Convolution2D(128, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(Convolution2D(128, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(MaxPooling2D(pool_size=(2,2), strides=(1,1)))  # 20 x 20
...
model.add(Convolution2D(256, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(Convolution2D(256, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(MaxPooling2D(pool_size=(2,2), strides=(1, 1)))  # 13 x 13
...
model.add(Convolution2D(256, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(Convolution2D(256, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(MaxPooling2D(pool_size=(2,2), strides=(1, 1)))  # 6 x 6
#model.add(Dropout(0.3))

# ------------------------------------------------------------------------------------------------------------------------------------------------ #

model.add(Convolution2D(512, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(Convolution2D(512, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))
model.add(MaxPooling2D(pool_size=(2,2), strides=(1, 1)))  # 1 x 1
...
model.load_weights(weightSavePath + "bestWeightsAtEpoch_034.h5")
