...
logger 			= '/media/AccessParag/Code/DNN_imageQuality_Estmn_loweKernels_Apr21.txt'
...
class myCallback(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        model.save_weights(weightSavePath + "bestWeights_qualityEstmn_lowKernels_latestModel.h5",overwrite=True)
        ...
        if mean_corr > self.best_mean_corr:
            ...
            model.save_weights(weightSavePath + "bestWeights_qualityEstmn_lowKernels_bestCorr.h5",overwrite=True)
...
#
# # model.add(Convolution2D(512, 3, 3, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))  # 7 x 15
# model.add(Convolution2D(1024, 1, 1, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))  # 5 x 13
# model.add(Convolution2D(1024, 1, 1, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))  # 3 x 11
# model.add(Convolution2D(1024, 1, 1, border_mode='valid', trainable=True, init=initialization, W_regularizer=l2(regularizer), subsample=(1, 1), activation = "relu"))  # 1 x 9
...
checkpointer = ModelCheckpoint(filepath = weightSavePath + "bestWeights_qualityEstmn_lowKernels_bestLoss.h5", monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
cb = myCallback()
history = LossHistory()
